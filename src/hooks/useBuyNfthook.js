        ...
        async () => {
          try {
            ...
            const estimateGas = await contractinstance.methods
              .buyNFTFromSale(nftAddress, tokenId, listId - 1)
  ...
  const handleBuy = useCallback(
    async (tokenId, listId, nftAddress, nftContract, callback, handleError) => {
      try {
        ...
        if (isapproved) {
          buyNFT(web3, contractinstance, callback, tokenId, listId, nftAddress, handleError);
        } else {
          setApprovalforall(
            web3,
            nftcontractinstance,
